...
class Cruddecease extends BaseController {
  ...
  function filteredgrid()
  {
    ...
    $filter->db->select("*, date_format(tarikhMati, '%d/%m/%Y') as 'dd', date_format(tarikhDaftarMati, '%d/%m/%Y') as 'rd'");
    $filter->db->from("kwkpp_kematian");
    $filter->db->join("kwkpp_ahli_khairat","kwkpp_ahli_khairat.ahliKhairatID=kwkpp_kematian.ahliKhairatID","LEFT");
    $filter->db->join("kwkpp_negeri","kwkpp_negeri.negeriCode=kwkpp_kematian.negeriID","LEFT");

    $filter->noPekerja = new inputField("No Pekerja", "noPekerja");
	$filter->noAhli = new inputField("No Kematian", "noMati");
...
//if ($this->uri->segment(3)==="update" && $this->on_success() || $this->uri->segment(5)==="update" && $this->on_success())	
//{
//$query = $this->db->query("UPDATE kwkpp_ahli_khairat SET statusAhli='WOW' WHERE ahliKhairatID='00001'");
//}
/*elseif($this->dataedit->_action==="insert")
{
	if ($this->uri->segment(3)==="insert" || $this->uri->segment(5)==="insert" || $this->uri->segment(3)==="update" || $this->uri->segment(5)==="update")
	{
		if($this->uri->segment(3)==="insert" || $this->uri->segment(3)==="update")
		{$kematianID=$this->uri->segment(4);}
		elseif($this->uri->segment(5)==="insert" || $this->uri->segment(5)==="update")
		{$kematianID=$this->uri->segment(6);}	
		$query = $this->db->query("SELECT ahliKhairatID FROM kwkpp_kematian WHERE kematianID='$kematianID'");
		if ($query->num_rows() > 0)
		{
		   ...
		   $query = $this->db->query("UPDATE kwkpp_ahli_khairat SET statusAhli='M' WHERE ahliKhairatID='$ahliKhairatID'");
		}

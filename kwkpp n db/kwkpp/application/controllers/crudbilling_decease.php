...
class Crudbilling_decease extends BaseController {

  var $data_type = null;   
  var $data = null;


	function Crudbilling_decease()
	{
  ...
  function index()
  {
    ...
    redirect("crudbilling_decease/filteredgrid");
  ...
  function filteredgrid()
  {
    ...
    //filter
    $filter = new DataFilter("Carian Bil Bulanan Sumbangan Kematian Sahaja");
    $filter->db->select("*");
    ...
    $filter->bulan->option("","");  
	$filter->bulan->options($this->DateTime_model->month_mm());
    ...
    $filter->tahun->option("","");  
	$filter->tahun->options($this->DateTime_model->yearBill_yyyy());
	//$filter->tahun->value=date("Y");
    ...
    $uri = "crudbilling_decease/dataedit/show/<#billingID#>";
    $uri_bayar = "crudpaybill/dataedit/create/<#ahliKhairatID#>/<#tahun#>/<#bulan#>/S1/<#amaun_bil#>";

    //grid
    $grid = new DataGrid("Senarai Bil Bulanan Sumbangan Kematian");
...
//echo $grid->output('bulan');
	...
	$grid->column("Jumlah Kematian", "mati");
	$grid->column("Amaun Bil", "amaun_bil");
	$grid->column("Baki Bawa Hadapan", "bakiBawaHadapan");	

    $grid->add("crudbilling_decease/dataedit/create");
    $grid->build();
    /**/
  ...
  {  
    ...
    $edit = new DataEdit("Bil Bulanan Sumbangan Kematian", "kwkpp_billing");
    $edit->back_uri = "crudbilling_decease/filteredgrid";
    ...
    $edit->tahun->option("","");  
	$edit->tahun->options($this->DateTime_model->yearBill_yyyy());
	$edit->tahun->insertValue = "tahun";
	$edit->tahun->rule = "required";

	$edit->mati = new inputField("Bil Kematian", "mati");
    $edit->mati->rule = "trim|required|numeric|max_length[4]";

	$edit->amaun_bil = new inputField("Amaun Bil", "amaun_bil");
    $edit->amaun_bil->rule = "trim|required|numeric|max_length[9]";

	$edit->bakiBawaHadapan = new inputField("Baki Bawa Hadapan", "bakiBawaHadapan");
    $edit->bakiBawaHadapan->rule = "trim|required|numeric|max_length[9]";
...
//eg: # $edit->wysiwyg->when = array('show','modify');  

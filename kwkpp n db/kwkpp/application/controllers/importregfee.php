...
class importregfee extends BaseController {

  var $data_type = null;   
  var $data = null;


	function importregfee()
	{
  ...
  ##### index #####
	...
	{	
		if(is_logged() && get_role()<='3')
		{
			...
			$this->_render('upload_form1', $data=array('error' => ' ' ));
	...
	{	
		...
		$this->load->library('upload', $config);
			...
			if (!$this->upload->do_upload())
			{
				...
				$this->_render('upload_form1', $error);
			}	
...
if($remark == "Ahli")
{
				$query_billID = $this->db->query("select billingID from kwkpp_billing where ahliKhairatID='".$khairatID."' and tahun='".$line3[0]."' and bulan='".$line3[1]."' and jenisBilCode='".$this->pendaftaran_bil_code."'");
				...
				$query = "INSERT INTO kwkpp_sumbangan SET ahliKhairatID='$khairatID', billingID='".$billID."',  jenisBilCode='".$this->pendaftaran_bil_code."', tahun='$line3[0]', bulan='$line3[1]', amaun='$line[5]', jenisBayaranCode='6'";
...
/*update table kwkpp_sumbangan with current billing if exist*/	//
				...
				$query_update = "UPDATE kwkpp_sumbangan set billingID='".$billID."', amaun='".$line[5]."' where ahliKhairatID='".$khairatID."' and jenisBilCode='".$this->sumbangan_bil_code."' and tahun='".$line3[0]."' and bulan='".$line3[1]."'";
				...
				$this->_render('upload_success1', $userdata);

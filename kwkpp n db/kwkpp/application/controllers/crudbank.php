...
class Crudbank extends BaseController {
  ...
  function index()
  {
    ...
    redirect("crudbank/filteredgrid");
  ...
  function filteredgrid()
  {
    ...
    //filter
    $filter = new DataFilter("Carian Bank");
    $filter->db->select("*");
    $filter->db->from("kwkpp_bank");
    //$filter->db->join("authors","authors.author_id=articles.author_id","LEFT");

    $filter->bankDesc = new inputField("Bank", "bankDesc");
	    
    $filter->buttons("reset","search");    
    $filter->build();
    

    $uri = "crudbank/dataedit/show/<#bankID#>";
    
    //grid
    $grid = new DataGrid("Senarai Bank");
    $grid->use_function("callback_test");
    $grid->order_by("bankDesc","asc");
    $grid->per_page = 25;
    $grid->use_function("substr");

	$grid->column_detail("Bank", "bankDesc", $uri);
	    
    $grid->add("crudbank/dataedit/create");
    $grid->build();
    /**/
   ...
   $this->_render("crud", $data, 
	...
	//$html = $this->load->view("crud", $data, true);
	//pdf_create($html, 'list', 'a4', 'landscape');	  
	}else{redirect("auth/login");}
  }
  ...
  {  
    ...
    $edit = new DataEdit("Bank", "kwkpp_bank");
    $edit->back_uri = "crudbank/filteredgrid";

    $edit->bankDesc = new inputField("Bank", "bankDesc");
    $edit->bankDesc->rule = "trim|required|max_length[30]";

...
class Crudpayee extends BaseController {
  ...
  function index()
  {
    ...
    redirect("crudpayee/filteredgrid");
  ...
  function filteredgrid()
  {
    ...
    //filter
    $filter = new DataFilter("Carian Pembayar");
    $filter->db->select("*");
    $filter->db->from("kwkpp_pembayar");
    //$filter->db->join("authors","authors.author_id=articles.author_id","LEFT");

    $filter->pembayarDesc = new inputField("Pembayar", "pembayarDesc");
	    
    $filter->buttons("reset","search");    
    $filter->build();
    

    $uri = "crudpayee/dataedit/show/<#pembayarID#>";
    
    //grid
    $grid = new DataGrid("Senarai Pembayar");
    $grid->use_function("callback_test");
    $grid->order_by("pembayarDesc","asc");
    $grid->per_page = 5;
    $grid->use_function("substr");

	$grid->column_detail("Pembayar", "pembayarDesc", $uri);
		
    $grid->add("crudpayee/dataedit/create");
    $grid->build();
    /**/
  ...
  function comments_edit()
  {
    ...
    $edit->back_uri = "crudpayee/comments_grid/$art_id/list";
  ...
  {  
    ...
    $edit = new DataEdit("Pembayar", "kwkpp_pembayar");
    $edit->back_uri = "crudpayee/filteredgrid";

    $edit->pembayarDesc = new inputField("Pembayar", "pembayarDesc");
    $edit->pembayarDesc->rule = "trim|required|max_length[25]";

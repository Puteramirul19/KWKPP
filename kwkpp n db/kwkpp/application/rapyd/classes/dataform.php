...
class DataForm {
        ...
        if (is_subclass_of($property, 'objField')){
          ...
          if (isset($this->$property_name->rule)){
            if ((strpos($this->$property_name->rule,"required")!==false) && !isset($this->$property_name->no_star) ){
              $this->$property_name->_required = "<font class=\"alert\"> *</font>";
            }
          }
  ...
  */
  function build_form(){
    ...
    foreach ($ordered_fields as $group=>$series_of_fields){
			...
			$gr["group_name"] = $group;
			$gr["group_tr"] = 'id="tr_'.underscore(strtolower($group)).'"';
  ...
  */
  function build(){
    ...
    $this->form_scripts = HTML::javascriptTag($this->_script[$this->_status]);

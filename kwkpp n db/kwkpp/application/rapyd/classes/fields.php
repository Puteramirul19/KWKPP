...
//MODIF 3 design path 
$ci =& get_instance();
define('RAPYD_FIELD_SYMBOL_NULL',           '<em>[ TIADA ]</em>');
  ...
  function _getValue(){
    ...
    } elseif ( (isset($this->data)) && ($this->data->loaded) && (!isset($this->request[$this->name])) && (isset($this->db_name)) )
    {
      ...
      if (isset($this->rel_id)){
        $this->value = $this->data->get_rel($this->rel_id, $this->db_name);
      } else {
    ...
    if (is_object($this->data)&& isset($this->db_name)){
      ...
      if (isset($this->rel_id)){
        ...
        if (isset($this->newValue)){
          $this->data->set_rel($this->rel_id, $this->db_name, $this->newValue);
        } else {
          $this->data->set_rel($this->rel_id, $this->db_name, $this->value);
        }

...
/*add function for update member type from AB to SH, also check for update from SH to AB but not allowed and create a billing for SH registration (refer to kwkpp_jenis_ahli).... should have column in db for the previous member type*/
$filename = explode('/',$this->_postprocess_uri);
if($filename[0] == "crudmembers"){
...
/*add function for insert NEW member, d func will check the maximum no Ahli to the appropriate type of member either AB or SH and +1 to have new NOAHLI....*/
...
/* This function will execute when user update decease member, it will update status as MD - Decease*/
...
/*when insert or update membership info*/
function crudmembers_onsuccess($ahliKhairatID)
{    
switch($this->_action){
case "insert":
$query = $this->db->query("select SUBSTRING(ak.tarikhMasuk,1,4) as 'tahun', SUBSTRING(ak.tarikhMasuk,6,2) as 'bulan', ja.amaunYuran as 'amaunYuran', ak.jenisAhliID as 'jenisAhliID' from kwkpp_ahli_khairat ak left join kwkpp_jenis_ahli ja on (ak.jenisAhliID=ja.jenisAhliID) where ahliKhairatID='".$ahliKhairatID."'");
if ($query->num_rows() > 0)
{
   foreach ($query->result() as $row)
   {
	  ...
	  $jenisAhliID = $row->jenisAhliID;
   }
/*create reg billing for new member*/   
...
/*get no Ahli 4 new member depends on the type of member*/
//$query = $this->db->query("LOCK TABLES kwkpp_ahli_khairat akr READ, kwkpp_ahli_khairat akw WRITE");

if($jenisAhliID==='1')
	{
		/*get max+1 SH no*/
		$query = $this->db->query("select max(noAhliSH)+1 as 'noAhliSH' from kwkpp_ahli_khairat");
		foreach ($query->result() as $row)
		{
		  $noAhliSH = $row->noAhliSH;
		  $update_value = "noAhliSH='".$noAhliSH."'";
		}
	}
elseif($jenisAhliID==='2')
	{
		/*get max+1 AB no*/
		$query = $this->db->query("select max(noAhliAB)+1 as 'noAhliAB' from kwkpp_ahli_khairat");
		foreach ($query->result() as $row)
		{
		  $noAhliAB = $row->noAhliAB;
		  $update_value = "noAhliAB='".$noAhliAB."'";
		}
	}
$query = $this->db->query("UPDATE kwkpp_ahli_khairat SET $update_value WHERE ahliKhairatID='$ahliKhairatID'");

//$query = $this->db->query("UNLOCK TABLES;");

break;
case "update":
/*give another no Ahli SH after they update the type of member to ahli seumur hidup*/
$query = $this->db->query("select jenisAhliID as 'jenisAhliID', noAhliAB, noAhliSH from kwkpp_ahli_khairat where ahliKhairatID='".$ahliKhairatID."'");
if ($query->num_rows() > 0)
{
   foreach ($query->result() as $row)
   {

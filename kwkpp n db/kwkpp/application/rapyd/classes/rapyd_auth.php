...
class rapyd_auth{
  ...
  function trylogin($username, $password, $cookie = true, $max_role=1) //
  {
	...
	/*
	$this->ci->encrypt->set_hash('md5');
	$this->ci->encrypt->hash($password, 'md5');
	*/
	...
	/***AD Login*/
	$bd = 0;
	$ad = ldap_connect ('tnb.my', '389'); // or die("Maaf, tiada akses ke Active Directory Server.");//die("Couldn't connect to AD!");
	ldap_set_option($ad, LDAP_OPT_PROTOCOL_VERSION, 3); // or die("Maaf, protokol tidak dibenarkan.");//die ("Could not set ldap protocol");
	//ldap_set_option($ad, LDAP_OPT_REFERRALS, 0) or die ("Could not set ldap protocol");
	$bd = ldap_bind($ad, $username."@tnb.my", $password);// or die("Maaf, nama pengguna atau kata laluan salah.");//die("Couldn't bind to AD!");
	/***AD Login*/
    ...
    if($return && $bd)
    {
	  ...
	  //$this->db->set("act_ip", $_SERVER['REMOTE_ADDR']);
      $this->db->where($this->field_username, $username);
  ...
  function trylogin_bycookie()
  {
    ...
    if (!$this->is_logged() && $cookie){
      ...
      return $this->trylogin( $auth_fields['username'] , true); //$auth_fields['password'],
    }
    return false;
  }

...
class CI_DB_active_record extends CI_DB_driver {
	...
	/**edited by me
	wherebetween1st_datetime &  wherebetween2nd_datetime
	for search in a range of date (hide the time, only show date)
	*/
	function wherebetween1st_datetime($key, $value = NULL, $type = 'AND ')
	{
		if ( ! is_array($key))
		{
			$key = array($key => $value);
	...
	function wherebetween2nd_datetime($key, $value = NULL, $type = 'AND ')
	{
		if ( ! is_array($key))
		{
			$key = array($key => $value);
		...
		foreach ($key as $k => $v)
		{
			...
			if ( ! is_null($v))
			{
				...
				$v = ' '.$this->escape($v);
				$v = substr($v,0,12)." 23:59:59'"; // put the latest time on the selected date
			}
